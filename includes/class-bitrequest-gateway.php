...
class WC_Gateway_Bitrequest extends WC_Payment_Gateway {
    ...
    /**
     * URL of a locally bundled coin icon, or '' when none is shipped.
     *
     * Customer-facing output uses ONLY this — never the CoinMarketCap CDN — so a
     * shopper's browser never contacts a third party while paying. Admin screens
     * still use the CDN, which is disclosed under "External services" in readme.txt.
     *
     * Drop 64x64 PNGs into assets/img/coins/ named after the coin key
     * (bitcoin.png, lightning.png, monero.png, ...). Missing files degrade to a
     * text-only label, so the plugin works with the directory empty.
     */
    public static function local_icon_url( string $coin ): string {
        $slug = self::is_erc20_token( $coin ) ? 'erc20-token' : $coin;
        if ( ! preg_match( '/^[a-z0-9-]+$/', $slug ) ) return '';
        $rel = 'assets/img/coins/' . $slug . '.png';
        return file_exists( BITREQUEST_WC_PATH . $rel ) ? BITREQUEST_WC_URL . $rel : '';
    }
...
public function process_admin_options() {
            ...
            <div class="br-dropdown" id="br-dropdown" role="combobox" aria-expanded="false" aria-haspopup="listbox" tabindex="0">
                ...
                <ul class="br-dropdown-menu" id="br-dropdown-menu" role="listbox">
                    <?php foreach ( $enabled as $coin => $cfg ) :
                        [ $label, $symbol, ] = self::coin_display_info( $coin, $cfg );
                        // Locally bundled only — no third-party request from the shopper's browser.
                        $icon = self::local_icon_url( $coin );
                    ?>
    ...
    public function enqueue_admin_assets( $hook ) {
        ...
        wp_enqueue_script( 'bitrequest-derive', BITREQUEST_WC_URL . 'assets/js/bitrequest-derive.js',
            [ 'br-bip39-utils', 'br-crypto-utils' ], BITREQUEST_WC_VERSION, true );
        wp_enqueue_style( 'bitrequest-admin-css', BITREQUEST_WC_URL . 'assets/css/bitrequest-admin.css', [], BITREQUEST_WC_VERSION );
